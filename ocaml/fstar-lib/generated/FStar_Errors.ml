...
let (errno : FStar_Errors_Codes.error_code -> Prims.int) =
  fun e ->
...
type error =
  (FStar_Errors_Codes.error_code * FStar_Errors_Msg.error_message *
    FStar_Compiler_Range_Type.range * Prims.string Prims.list)
type issue_level =
  | ENotImplemented 
  | EInfo 
  | EWarning 
...
type issue =
  ...
  issue_range: FStar_Compiler_Range_Type.range FStar_Pervasives_Native.option ;
...
let (__proj__Mkissue__item__issue_msg :
  issue -> FStar_Errors_Msg.error_message) =
  fun projectee ->
    match projectee with
...
let (__proj__Mkissue__item__issue_range :
  issue -> FStar_Compiler_Range_Type.range FStar_Pervasives_Native.option) =
  fun projectee ->
    match projectee with
    | { issue_msg; issue_level = issue_level1; issue_range; issue_number;
...
let (issue_message : issue -> FStar_Errors_Msg.error_message) =
  fun i ->
    ...
    FStar_Compiler_List.op_At i.issue_msg uu___
  ...
  fun print_hdr ->
    fun issue1 ->
          ...
          let uu___1 =
            ...
            let uu___3 =
              let uu___4 =
                let uu___5 =
                  let uu___6 =
                    let uu___7 = FStar_Pprint.doc_of_string ":" in
                    FStar_Pprint.op_Hat_Hat uu___7 FStar_Pprint.hardline in
                  FStar_Pprint.op_Hat_Hat atrng uu___6 in
                FStar_Pprint.op_Hat_Hat num_opt uu___5 in
              FStar_Pprint.op_Hat_Hat level_header uu___4 in
            FStar_Pprint.op_Hat_Hat uu___2 uu___3 in
          FStar_Pprint.op_Hat_Hat uu___ uu___1
        else FStar_Pprint.empty in
      let seealso =
        match r with
        | FStar_Pervasives_Native.Some r1 when
            (let uu___ = FStar_Compiler_Range_Type.def_range r1 in
             let uu___1 = FStar_Compiler_Range_Type.use_range r1 in
             uu___ <> uu___1) &&
              (let uu___ = FStar_Compiler_Range_Type.def_range r1 in
               let uu___1 =
               ...
               uu___ <> uu___1)
            ->
            let uu___ = FStar_Pprint.doc_of_string "See also" in
            let uu___1 =
              let uu___2 = FStar_Pprint.blank Prims.int_one in
              let uu___3 =
                let uu___4 = FStar_Compiler_Range_Ops.string_of_range r1 in
      ...
      let ctx =
        ...
        | h::t when FStar_Options.error_contexts () ->
            let d1 s =
              let uu___ = FStar_Pprint.doc_of_string "> " in
              let uu___1 = FStar_Pprint.doc_of_string s in
  ...
  fun i1 ->
    fun i2 ->
      ...
      | (FStar_Pervasives_Native.Some r1, FStar_Pervasives_Native.Some r2) ->
          FStar_Compiler_Range_Ops.compare_use_range r1 r2
let (dummy_ide_rng : FStar_Compiler_Range_Type.rng) =
...
let (fixup_issue_range :
  ...
  fun rng ->
    let rng1 =
      match rng with
      | FStar_Pervasives_Native.None ->
          FStar_Compiler_Effect.op_Bang fallback_range
      | FStar_Pervasives_Native.Some range ->
          let use_rng = FStar_Compiler_Range_Type.use_range range in
          let use_rng' =
            if
              (use_rng <> FStar_Compiler_Range_Type.dummy_rng) &&
                (use_rng <> dummy_ide_rng)
            then use_rng
            else
              (let uu___1 =
                 let uu___2 = FStar_Compiler_Effect.op_Bang fallback_range in
                 FStar_Pervasives_Native.uu___is_Some uu___2 in
               if uu___1
               then
                 let uu___2 =
                   let uu___3 = FStar_Compiler_Effect.op_Bang fallback_range in
                   FStar_Pervasives_Native.__proj__Some__item__v uu___3 in
                 FStar_Compiler_Range_Type.use_range uu___2
               else use_rng) in
          let uu___ = FStar_Compiler_Range_Type.set_use_range range use_rng' in
          FStar_Pervasives_Native.Some uu___ in
    FStar_Compiler_Util.map_opt rng1 maybe_bound_range
...
let (lookup :
  FStar_Errors_Codes.error_code -> FStar_Errors_Codes.error_setting) =
  fun err ->
...
let info :
  'posut .
    'posut FStar_Class_HasRange.hasRange ->
      'posut ->
        unit -> Obj.t FStar_Errors_Msg.is_error_message -> Obj.t -> unit
  =
  fun uu___ ->
    fun r ->
      fun uu___1 ->
        fun uu___2 ->
          fun msg ->
            let rng = FStar_Class_HasRange.pos uu___ r in
            let rng1 = fixup_issue_range (FStar_Pervasives_Native.Some rng) in
            let msg1 = FStar_Errors_Msg.to_doc_list uu___2 msg in
...
let raise_error :
  'a 'posut .
    'posut FStar_Class_HasRange.hasRange ->
      'posut ->
        FStar_Errors_Codes.error_code ->
          unit -> Obj.t FStar_Errors_Msg.is_error_message -> Obj.t -> 'a
  =
  fun uu___ ->
    fun r ->
      fun e ->
...
let raise_error0 :
  ...
  fun e ->
    fun uu___ ->
      fun uu___1 ->
        fun msg ->
          raise_error FStar_Class_HasRange.hasRange_range
            FStar_Compiler_Range_Type.dummyRange e () uu___1 msg
let (log_issue0 :
  FStar_Errors_Codes.error_code ->
    unit -> Obj.t FStar_Errors_Msg.is_error_message -> Obj.t -> unit)
  =
  fun e ->
    fun uu___ ->
      fun uu___1 ->
        fun msg ->
          log_issue FStar_Class_HasRange.hasRange_range
            FStar_Compiler_Range_Type.dummyRange e () uu___1 msg
let diag0 : 't . 't FStar_Errors_Msg.is_error_message -> 't -> unit =
  fun uu___ ->
    fun msg ->
      diag FStar_Class_HasRange.hasRange_range
        FStar_Compiler_Range_Type.dummyRange () (Obj.magic uu___)
        (Obj.magic msg)
...
let raise_error_doc :
  'a .
    FStar_Compiler_Range_Type.range ->
      FStar_Errors_Codes.error_code -> FStar_Errors_Msg.error_message -> 'a
  =
  fun rng ->
    fun code ->
      fun msg ->
        raise_error FStar_Class_HasRange.hasRange_range rng code ()
          (Obj.magic FStar_Errors_Msg.is_error_message_list_doc)
          (Obj.magic msg)
let (log_issue_doc :
  FStar_Compiler_Range_Type.range ->
    FStar_Errors_Codes.error_code -> FStar_Errors_Msg.error_message -> unit)
  =
  fun rng ->
    fun code ->
      fun msg ->
        log_issue FStar_Class_HasRange.hasRange_range rng code ()
          (Obj.magic FStar_Errors_Msg.is_error_message_list_doc)
          (Obj.magic msg)
let raise_error_text :
  'a .
    FStar_Compiler_Range_Type.range ->
      FStar_Errors_Codes.error_code -> Prims.string -> 'a
  =
  fun rng ->
    fun code ->
      fun msg ->
