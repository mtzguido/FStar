  ...
  fun f ->
    ...
    let matches =
      FStar_Compiler_List.map
        (fun ext ->
           let lext = FStar_Compiler_String.length ext in
           let l = FStar_Compiler_String.length f in
           let uu___ =
             (l > lext) &&
               (let uu___1 =
                  FStar_Compiler_String.substring f (l - lext) lext in
                uu___1 = ext) in
           if uu___
           then
             let uu___1 =
               FStar_Compiler_String.substring f Prims.int_zero (l - lext) in
...
let (is_interface : Prims.string -> Prims.bool) =
  fun f ->
    let uu___ =
      FStar_Compiler_String.get f
        ((FStar_Compiler_String.length f) - Prims.int_one) in
    uu___ = 105
...
let (lowercase_module_name : Prims.string -> Prims.string) =
  fun f ->
    ...
    FStar_Compiler_String.lowercase uu___
...
let (string_of_lid : FStar_Ident.lident -> Prims.bool -> Prims.string) =
  fun l ->
    fun last ->
      ...
      FStar_Compiler_String.concat "." names
                ...
                ((let uu___2 = FStar_Compiler_Effect.op_Bang dbg in
                  ...
                  then
                    ...
                    FStar_Compiler_Util.print3
                      ...
                      (FStar_Compiler_String.concat ", " all_files_0) uu___3
                      (FStar_Compiler_String.concat ", "
                           ...
                           ((let uu___6 = FStar_Compiler_Effect.op_Bang dbg in
                             ...
                             then
                               FStar_Compiler_Util.print1
                                 ...
                                 (FStar_Compiler_String.concat ", " all_files)
                             else ());
                            (all_files, widened1)))))
let (phase1 :
  files_for_module_name ->
  ...
  fun outc ->
    fun deps1 ->
      let sort_output_files orig_output_file_map =
        ...
        let mark_visiting lc_module_name =
          let ml_file_opt =
            FStar_Compiler_Util.smap_try_find remaining_output_files
              lc_module_name in
          FStar_Compiler_Util.smap_remove remaining_output_files
            lc_module_name;
      ...
      let no_fstar_stubs_file s =
        ...
        let uu___ =
          ((FStar_Compiler_String.length s) >= l1) &&
            (let uu___1 = FStar_Compiler_String.substring s Prims.int_zero l1 in
             uu___1 = s1) in
        if uu___
        then
          let uu___1 =
      ...
      let output_file ext fst_file =
        let basename =
          let uu___ =
            let uu___1 = FStar_Compiler_Util.basename fst_file in
            check_and_strip_suffix uu___1 in
          FStar_Compiler_Option.get uu___ in
  ...
  fun outc ->
    fun fn ->
      fun deps1 ->
        ...
        match uu___ with
        | FStar_Pervasives_Native.Some "make" -> print_make outc deps1
...
let (print : deps -> unit) =
  fun deps1 ->
    let uu___ = FStar_Options.output_deps_to () in
    match uu___ with
