...
let is_norm_request_head (t : term) : option norm_request_kind =
  match (U.un_uinst t).n with
  | Tm_fvar fv when S.fv_eq_lid fv PC.normalize_term -> Some NormalizeTerm
  | Tm_fvar fv when S.fv_eq_lid fv PC.normalize -> Some Normalize
  | Tm_fvar fv when S.fv_eq_lid fv PC.norm -> Some Norm
  | _ -> None
    ...
    fun cfg env stack t ->
        ...
        match t.n with
          ...
          | _ when
              ...
              Some? (is_norm_request_head t) ->
            handle_norm_request cfg env stack (Some?.v (is_norm_request_head t)) t
...
and handle_norm_request (cfg:Cfg.cfg) env stack (k : norm_request_kind) (hd : term) =
  ...
  if debug then
    BU.print1 "GG handle norm request %s\n" (show hd);
  if debug then
    BU.print1 "GG handle norm request stack = %s\n" (show stack);
  ...
  let term_steps_stack =
    ...
    | Normalize, Arg (Clos (a_env, a_t, _, _) , _, _) :: stack' ->
      ...
      Some (closure_as_term cfg a_env a_t, steps), stack'
    | Norm, Arg (Clos (s_env, s_t, _, _), _, _) :: Arg (Clos (_, _, _, _) , _, _) :: Arg (Clos (a_env, a_t, _, _) , _, _) :: stack' ->
      ...
      | Some s ->
        ...
        Some (closure_as_term cfg a_env a_t, s), stack'
      | None -> None, stack
      end
    | _ ->
      if debug then
