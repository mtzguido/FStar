...
ARG CI_NO_KARAMEL=
ARG RESOURCEMONITOR=
RUN Z3_LICENSE="$(opam config expand '%{prefix}%')/.opam-switch/sources/z3.4.8.5/LICENSE.txt" \
    CI_NO_KARAMEL=$CI_NO_KARAMEL \
    .docker/build/build-standalone.sh $CI_TARGET $CI_THREADS $CI_BRANCH
